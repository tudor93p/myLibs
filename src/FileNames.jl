...
function fname(name::String, func::Function, args...; ignore=[])::Function
	...
	#	restrict = filter(p->!(p.first in ignore),p)

	strings = if length(args)==1

		[params_to_string(func(
													 args[1]
		...
		params_to_string.(func(args...))
								
					end	



	prefix = mkpath(join([ROOT;name;filter(!isempty,vcat(strings...))...],"/"))
...
function fname(M::Module,n::Int64=1,args...; kwargs...)::Function
	...
	fname(Base.fullname(M)[end-n+1:end],M.params_digits,args...; kwargs...)
...
function typical_params_digits(usedkeys=[], digits=NamedTuple())

	return P -> (P, usedkeys, digits)
