...
function figure101(ax, ax_)
	...
	show_transversal = true  
	show_transversal = false



	show_longitudinal = !show_transversal 
...
#	MUS0 .+= (2rand(length(MUS0)).-1)*0.003
...
#		@assert G_ret(A,1,B,1)' ≈ G_adv(B,1,A,1)
		...
		if !isnothing(k)
			...
			[a.plot([mu,mu],[0,1.1],lw=2,c=col,alpha=0.8,linestyle=":") for a in [ax2,ax4]]
			...
			bondT3 = ObservablesFromGF.BondTransmission(G_ret, G_adv, hoppings_bonds, inds_bonds, se, T3_lead...) 
			
#			ObservablesFromGF.SiteTransmission(G_ret, hoppings_bonds, inds_bonds, Rs_bonds, se, B, 2;dim=2)
...
#					d > 1e-3 && @warn string("Mean transersal difference: ",dig3(d))
				...
				if show_longitudinal 
					...
					println("  Net longitudinal: 2*",dig3(s))
...
#				I = sort(Utils.flatmap(LAR[:IndsAtomsOfLayer], 2:LAR[:NrLayers]-1))
				...
				if show_longitudinal 
					...
					pr_s2 = string("Net longitudinal method 2: 2*", dig3(s2)) 
